...
type StaffField = keyof Omit<StaffRow, 'id'>
...
const COLUMNS: { key: StaffField; label: string; cellClassName: string }[] = [
  { key: 'orgStructureCode', label: 'Код оргструктуры', cellClassName: styles.colWeight },
  { key: 'unitName', label: 'Наименование', cellClassName: styles.colGoal },
  { key: 'head', label: 'Руководитель', cellClassName: styles.colSurname },
  { key: 'businessUnit', label: 'Бизнес юнит', cellClassName: styles.colWeight },
  { key: 'functionalBlockCurator', label: 'Куратор ф. блока', cellClassName: styles.colGoal },
]

const EDIT_FIELDS: RegistryRowField<StaffRow>[] = COLUMNS.map((col) => ({
  key: col.key,
  label: col.label,
  placeholder: '',
  multiline: col.key === 'unitName' || col.key === 'functionalBlockCurator',
}))

const DEFAULT_PAGE_SIZE = 15

export function StaffPage() {
  const [state, setState] = useState<{ rows: StaffRow[] }>({ rows: [] })
  ...
  const startEdit = useCallback((row: StaffRow) => {
  ...
  return (
    <div className={styles.page}>
      <header className={styles.hero}>
        <h1 className={styles.title}>Штатное расписание</h1>
      </header>
                ...
                pageRows.map((row) => (
                  <tr key={row.id}>
                    {COLUMNS.map((col) => {
                      ...
                      const multiline = col.key === 'unitName' || col.key === 'functionalBlockCurator'
                      return (
      ...
      <EditRegistryRowModal<StaffRow>
        open={editingRowId !== null}
        title={isAddingNewRow ? 'Новая запись' : 'Редактирование записи'}
        row={editingDraft}
        fields={EDIT_FIELDS}
        formId="edit-staff-row-form"
        ariaTitleId="edit-staff-row-modal-title"
        fieldIdPrefix="edit-staff"
        onSave={saveEdit}
        onCancel={cancelEdit}
      />
